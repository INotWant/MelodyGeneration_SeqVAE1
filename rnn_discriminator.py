...
class BidirectionalLstmDiscriminator(object):
    ...
    def discriminate(self, inputs, labels=None, dropout_keep_prob=1.0):
        ...
        with tf.variable_scope(self.name_or_scope, reuse=tf.AUTO_REUSE):
            ...
            if labels is not None:
                ...
                with tf.variable_scope("loss"):
                    ...
                    self.loss_scalar = tf.summary.scalar("d_loss", self.loss)
                # accuracy
                self.accuracy = tf.reduce_sum(labels * tf.one_hot(predictions, 2)) / self._batch_size
                self.accuracy_scalar = tf.summary.scalar("d_accuracy", self.accuracy)
